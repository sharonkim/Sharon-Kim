...
SELECT customer.first_name as first, customer.last_name as last, customer.email as email, CONCAT_WS(' ', address.address, city.city, address.postal_code) as address
FROM customer
...
FROM film
JOIN film_category ON film.film_id = film_category.film_id
JOIN category ON film_category.category_id = category.categor_id
